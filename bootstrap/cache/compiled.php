...
class SessionGuard implements StatefulGuard, SupportsBasicAuth
{
    ...
    public function logoutOtherDevices($password, $attribute = 'password')
    {
        ...
        $result = tap($this->user()->forceFill([$attribute => Hash::make($password)]))->save();
...
class Validator implements ValidatorContract
{
    ...
    protected $implicitRules = ['Accepted', 'Filled', 'Present', 'Required', 'RequiredIf', 'RequiredUnless', 'RequiredWith', 'RequiredWithAll', 'RequiredWithout', 'RequiredWithoutAll'];
    protected $dependentRules = ['After', 'AfterOrEqual', 'Before', 'BeforeOrEqual', 'Confirmed', 'Different', 'ExcludeIf', 'ExcludeUnless', 'ExcludeWithout', 'Gt', 'Gte', 'Lt', 'Lte', 'RequiredIf', 'RequiredUnless', 'RequiredWith', 'RequiredWithAll', 'RequiredWithout', 'RequiredWithoutAll', 'Same', 'Unique'];
...
class Blueprint
{
    ...
    {
        $this->columns[] = $column = new ForeignIdColumnDefinition($this, ['type' => 'bigInteger', 'name' => $column, 'autoIncrement' => false, 'unsigned' => true]);
        return $column;
    }
    public function foreignIdFor($model, $column = null)
    {
        ...
        return $model->getKeyType() === 'int' && $model->incrementing ? $this->foreignId($column ?: $model->getForeignKey()) : $this->foreignUuid($column ?: $model->getForeignKey());
    }
    ...
    {
        return $this->columns[] = new ForeignIdColumnDefinition($this, ['type' => 'uuid', 'name' => $column]);
    }
...
class Collection extends BaseCollection implements QueueableCollection
{
    ...
    public function loadCount($relations)
    {
        ...
        $models = $this->first()->newModelQuery()->whereKey($this->modelKeys())->select($this->first()->getKeyName())->withCount(...func_get_args())->get()->keyBy($this->first()->getKeyName());
